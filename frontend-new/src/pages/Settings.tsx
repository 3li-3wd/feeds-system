...
export default function SettingsPage() {
    ...
    const handleBackup = async () => {
        try {
            ...
            showToast(error.message || "حدث خطأ أثناء إنشاء النسخة الاحتياطية", "error")
        }
    }
    ...
    const handleRestore = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!confirm("تحذير: استعادة نسخة احتياطية سيؤدي لاستبدال جميع البيانات الحالية. هل أنت متأكد؟")) {
            e.target.value = ""
            return
        }
        ...
        try {
            ...
            await adminApi.restore(file)
            showToast("تمت استعادة البيانات بنجاح. سيتم إعادة تحميل الصفحة.", "success")
            setTimeout(() => {
                window.location.reload()
            }, 2000)
        } catch (error: any) {
            showToast(error.message || "حدث خطأ أثناء استعادة النسخة الاحتياطية", "error")
        } finally {
            e.target.value = ""
        }
    }
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <Button
                                        ...
                                        className="gap-2 w-full border-muted-foreground/20"
                                    >
