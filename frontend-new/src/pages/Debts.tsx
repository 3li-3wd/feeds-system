...
export default function DebtsPage() {
    ...
    const loadDebts = async () => {
        try {
            ...
            // Backend returns: customer_id, customer_name, phone_number, total_invoices, total_paid, debt, unpaid_invoices
            const mappedDebts: Debt[] = data.map((d: any) => ({
                id: d.customer_id,
                customerName: d.customer_name || "بدون اسم",
                phone: d.phone_number || "---",
                remainingAmount: Number(d.debt) || 0,
                unpaidInvoices: Number(d.unpaid_invoices) || 0
            }))
            setDebts(mappedDebts)
        } catch (err: any) {
            console.error('Load debts error:', err)
            showToast("فشل تحميل الديون", "error")
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const openDetailsModal = async (debt: Debt) => {
        ...
        setDebtDetails(null) // Reset details

        try {
            const details = await debtsApi.getDebtDetails(debt.id)
            setDebtDetails(details)
        } catch (err: any) {
            console.error('Load debt details error:', err)
            showToast("فشل تحميل تفاصيل الدين", "error")
    ...
    const openPaymentModal = async (debt: Debt) => {
        setSelectedDebt(debt)
        setPaymentAmount("")
        setSelectedInvoiceId("")
        setIsPaymentModalOpen(true)

        // Ensure we have invoice details to show in dropdown
        if (!debtDetails || debtDetails.customer.id !== debt.id) {
            setIsLoadingDetails(true)
            try {
                const details = await debtsApi.getDebtDetails(debt.id)
                setDebtDetails(details)
            } catch (err) {
                console.error("Failed to load details for payment modal:", err)
    ...
    const handlePaymentSubmit = async (e: React.FormEvent) => {
        ...
        if (!selectedInvoiceId) {
            showToast("يرجى اختيار الفاتورة", "error")
            return
        }

        const selectedInvoice = debtDetails?.invoices.find(inv => inv.id.toString() === selectedInvoiceId)
        ...
        if (amount > selectedInvoice.remaining) {
            showToast("المبلغ أكبر من المتبقي في الفاتورة", "error")
            return
        }
        ...
        try {
            ...
            loadDebts()
            if (selectedDebt) {
                const details = await debtsApi.getDebtDetails(selectedDebt.id)
                setDebtDetails(details)
            }
        } catch {
            ...
            >
                {isLoadingDetails ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-4 rounded-lg bg-muted/50">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-sm">
                                <div>
                                    ...
                                    <span className="font-medium">{debtDetails.customer?.name || selectedDebt?.customerName}</span>
                                </div>
                                <div>
                                    <span className="text-muted-foreground">الهاتف: </span>
                                    <span className="font-medium" dir="ltr">
                                        {debtDetails.customer?.phone || selectedDebt?.phone}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Invoices */}
                        <div>
                            <h4 className="font-semibold text-foreground mb-3">الفواتير غير المسددة</h4>
                            <div className="space-y-2">
                                {debtDetails.invoices && debtDetails.invoices.length > 0 ? (
                                    debtDetails.invoices.map((inv) => (
                                        <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg bg-card border">
                                            <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">{inv.date ? new Date(inv.date).toLocaleDateString("ar-SA") : "---"}</p>
                                            </div>
                                ...
                                <div className="space-y-2">
                                    {debtDetails.payments.map((payment) => (
                                        <div key={payment.id} className="flex items-center justify-between p-3 rounded-lg bg-success/5">
                                            <span className="text-sm">{payment.date ? new Date(payment.date).toLocaleDateString("ar-SA") : "---"}</span>
            ...
            <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title="تسجيل دفعة جديدة">
                {selectedDebt && (
                    <form onSubmit={handlePaymentSubmit} className="space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label>اختيار الفاتورة *</Label>
                                <select
                                    className="w-full p-2 border rounded-md bg-background text-sm"
                                    value={selectedInvoiceId}
                                    onChange={(e) => {
                                        setSelectedInvoiceId(e.target.value)
                                        const inv = debtDetails?.invoices.find(i => i.id.toString() === e.target.value)
                                        if (inv) setPaymentAmount(inv.remaining.toString())
                                    }}
                                    required
                                >
                                    ...
                                    {debtDetails?.invoices.map(inv => (
                                        <option key={inv.id} value={inv.id}>
                                            {inv.invoiceNumber} - المتبقي: {formatCurrency(inv.remaining)}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder="0"
                                    max={debtDetails?.invoices.find(i => i.id.toString() === selectedInvoiceId)?.remaining || 0}
                                />
                                {selectedInvoiceId && (
                                    <p className="text-xs text-muted-foreground italic">
                                        الحد الأقصى لهذه الفاتورة: {formatCurrency(debtDetails?.invoices.find(i => i.id.toString() === selectedInvoiceId)?.remaining || 0)}
                                    </p>
                                )}
                            </div>
                        </div>
